function plotAnalysis_2d(front, timeVals, linearFrames)
...
subplot(2,2,1);
	plot(wavenum(reliableWaves)-1, front.growthRates(reliableWaves), 'b');

	sFactor = max(front.growthRates(reliableWaves)) / max(front.growthCorr(reliableWaves));
	hold on;
	plot(wavenum(reliableWaves)-1, front.growthCorr(reliableWaves)*sFactor*.75, 'r');
	hold off;
	grid on;
