...
if nargin == 3;
    ...
    pos{nQuantLevels} = zeros([3 grid],GPUdouble);
    mass{nQuantLevels} = GPUdouble(finestRho);
    % Generate initial centers for every grid location
    for n = 1:3
        if numel(dGrid{n}) > 1
            pos{nQuantLevels}(n,:,:,:) = cumsum(dGrid{n}, n) - .5*dGrid{n}(1,1,1);
            mass{nQuantLevels} = mass{nQuantLevels} .* GPUdouble(dGrid{n}); % Compute mass = mass * dv 
        else
...
mass{nQuantLevels} = GPUdouble(mass{nQuantLevels});
...
for ct = (nQuantLevels-1):-1:1
    ...
    mass{ct} = GPUdouble(zeros(newgrid));
