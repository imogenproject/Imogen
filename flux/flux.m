...
%        mdir = 
    ...
    for i = i0:(i0+2)
        if rots(i) > 0
            xchgIndices(mass, mom, ener, mag, grav, rots(i));
        end
        if run.fluid.ACTIVE && fdir(i) > 0; relaxingFluid(run, mass, mom, ener, mag, grav, fdir(i));
%        if run.magnet.ACTIVE; magnetFlux(run, mass, mom, mag, directVec(n), magneticIndices(n,:)); end
    ...
    for i = i0:(i0+3)
        % rotate(rots(i)) once we get rotation working.
...
for i = 1:8
    s{i}.arrayIndexExchange(toex);
    if i < 6; s{i}.store.arrayIndexExchange(toex); end
end
